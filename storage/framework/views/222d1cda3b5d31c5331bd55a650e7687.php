...
<?php if($needsShopSelection ?? false): ?>
...
<div class="pos-layout">
        ...
        <?php $__currentLoopData = $groups; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $key => $label): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
          <li>
            <a href="<?php echo e(route('mt.pos.index', ['g'=>$key])); ?>"
               data-group="<?php echo e($key); ?>"
               style="text-decoration:none;display:block;padding:5px 8px;margin-bottom:1px;font-size:12px;<?php echo e($g===$key ? 'background:#333;color:#fff;font-weight:700;' : 'color:#333;'); ?>"
               class="js-pos-tab">
              <?php echo e($label); ?>
      ...
      <div id="sidebarCompaniesContainer">
        ...
        <?php else: ?>
          <ul style="list-style:none;padding-left:0;margin-top:6px;max-height:600px;overflow:auto;" id="sidebarCompanies">
            <li>
              <a href="<?php echo e(route('mt.pos.index', ['g'=>$g])); ?>"
                 data-company=""
                 style="text-decoration:none;display:block;padding:5px 8px;margin-bottom:1px;font-size:12px;<?php echo e($c === '' ? 'background:#333;color:#fff;font-weight:700;' : 'color:#333;'); ?>"
            ...
            <?php $__currentLoopData = ($companies ?? []); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $co): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
              <li>
                <a href="<?php echo e(route('mt.pos.index', ['g'=>$g,'c'=>$co->id])); ?>"
                   data-company="<?php echo e($co->id); ?>"
                   style="text-decoration:none;display:block;padding:5px 8px;margin-bottom:1px;font-size:12px;<?php echo e((string)$c === (string)$co->id ? 'background:#333;color:#fff;font-weight:700;' : 'color:#333;'); ?>"
                   class="js-pos-company">
                  <?php echo e($co->name); ?>
    ...
    <div class="pos-products-inner">
      <div style="display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:8px;margin-bottom:8px;">
        <div style="display:flex;gap:8px;align-items:center;">
          ...
          <?php if($canSeeCost): ?>
            <a href="<?php echo e(route('mt.purchases.create')); ?>" style="padding:3px 8px;background:#080;color:#fff;font-size:11px;font-weight:700;text-decoration:none;border:1px solid #060;">+ Stock In</a>
          <?php endif; ?>
        </div>
        <div style="display:flex;gap:4px;align-items:center;">
          <input id="posSearch" list="posSuggestList" placeholder="Search..." style="padding:4px 8px;border:1px solid #999;font-size:12px;min-width:180px;" value="<?php echo e($q); ?>">
        ...
        <?php if($g === 'leftover'): ?>
          <div class="pos-product-list">
            ...
            <?php $__empty_1 = true; $__currentLoopData = ($leftovers ?? []); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $lf): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
              ...
              <div class="pos-product-row js-pos-product-row" data-product-id="<?php echo e($p?->id ?? ''); ?>" style="grid-template-columns:1.4fr 0.6fr 1.2fr;">
                <div>
                  <div class="pos-product-name"><?php echo e($p?->name ?? 'Unknown'); ?></div>
                  <div class="pos-product-meta"><?php echo e($p?->company?->name ?? '-'); ?> • <?php echo e($p?->category?->name ?? '-'); ?></div>
                </div>
                <div style="text-align:right;font-weight:700;"><?php echo e(number_format($w,2)); ?>×<?php echo e(number_format($l,2)); ?></div>
                <div class="pos-product-actions">
                  <form method="POST" action="<?php echo e(route('mt.pos.leftover_add_full')); ?>" class="js-pos-ajax-form">
                    <?php echo csrf_field(); ?>
                  ...
                  <form method="POST" action="<?php echo e(route('mt.pos.leftover_add_cut')); ?>" class="flex gap-8 items-center js-pos-ajax-form">
                    <?php echo csrf_field(); ?>
                    <input type="hidden" name="leftover_id" value="<?php echo e($lf->id); ?>">
        ...
        <?php else: ?>
          ...
          <div class="pos-product-list">
            ...
            <?php $__currentLoopData = ($products ?? []); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $p): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
              ...
              <div class="pos-product-row js-pos-product-row" data-product-id="<?php echo e($p->id); ?>">
                <div>
                  ...
                  <div class="pos-product-meta">
                    ...
                    <?php if($showSheetButtons): ?> • Sheet: <?php echo e($fullW); ?>×<?php echo e($fullL); ?> <?php endif; ?>
                  </div>
                </div>
                <div class="pos-product-price">
                  <input
                    type="number"
                    step="0.1"
                    min="<?php echo e(number_format($minSell, 1, '.', '')); ?>"
                    value=""
                    placeholder="<?php echo e(number_format($sell, 1, '.', '')); ?>"
                    class="pos-product-sell-input js-product-sell"
                    data-default="<?php echo e(number_format($sell, 1, '.', '')); ?>"
                    data-min="<?php echo e(number_format($minSell, 1, '.', '')); ?>">
                  <?php if($maxDiscountPercent > 0): ?>
                    <div class="pos-product-sell-note">Max <?php echo e(rtrim(rtrim(number_format($maxDiscountPercent, 2, '.', ''), '0'), '.')); ?>% off</div>
                  <?php endif; ?>
                </div>
                <div class="pos-product-actions">
                  <?php if(!$showSheetButtons): ?>
                    <form method="POST" action="<?php echo e(route('mt.pos.add')); ?>" class="js-pos-price-form js-pos-ajax-form">
                      <?php echo csrf_field(); ?>
                      <input type="hidden" name="product_id" value="<?php echo e($p->id); ?>">
                      <input type="hidden" name="selling_price" value="<?php echo e(number_format($sell, 1, '.', '')); ?>" class="js-product-sell-hidden">
                  ...
                  <?php else: ?>
                    <form method="POST" action="<?php echo e(route('mt.pos.add_cut')); ?>" class="js-pos-price-form js-pos-ajax-form">
                      <?php echo csrf_field(); ?>
                      <input type="hidden" name="product_id" value="<?php echo e($p->id); ?>">
                      <input type="hidden" name="selling_price" value="<?php echo e(number_format($sell, 1, '.', '')); ?>" class="js-product-sell-hidden">
                    ...
                    <form method="POST" action="<?php echo e(route('mt.pos.add_cut')); ?>" class="flex gap-8 items-center js-pos-price-form js-pos-ajax-form">
                      <?php echo csrf_field(); ?>
                      <input type="hidden" name="product_id" value="<?php echo e($p->id); ?>">
                      <input type="hidden" name="selling_price" value="<?php echo e(number_format($sell, 1, '.', '')); ?>" class="js-product-sell-hidden">
                      <input type="hidden" name="full_w_ft" value="<?php echo e($fullW); ?>">
  ...
  <div class="pos-cart" style="border-radius:0;border:1px solid #ccc;">
    <div style="padding:6px 10px;background:#eee;border-bottom:1px solid #ccc;display:flex;justify-content:space-between;align-items:center;">
      <strong id="cartHeaderCount" style="font-size:11px;text-transform:uppercase;color:#333;">Cart (<?php echo e(count($cartRows ?? [])); ?>)</strong>
      <form method="POST" action="<?php echo e(route('mt.pos.clear')); ?>" class="js-pos-ajax-form js-pos-clear-form" onsubmit="return confirm('Clear all items from cart?')">
        <?php echo csrf_field(); ?>
      ...
      <?php $__empty_1 = true; $__currentLoopData = ($cartRows ?? []); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $r): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
        <div class="pos-cart-row">
          <div class="pos-cart-row-top">
            <div class="pos-cart-row-name"><?php echo e($r['name']); ?></div>
            <div class="pos-cart-row-controls">
            <input class="js-cart-qty pos-cart-inline-input" data-row="<?php echo e($r['row_id']); ?>" value="" type="number" step="1" min="1" title="Qty" placeholder="Qty">
            <span style="color:#94a3b8;font-size:11px;">x</span>
            <span class="pos-cart-field-label">Sell</span>
            <input class="js-cart-price pos-cart-inline-input pos-cart-price-input" data-row="<?php echo e($r['row_id']); ?>" value="<?php echo e(number_format((float)$r['price'],1,'.','')); ?>" type="number" step="0.1" min="0" style="width:72px;" title="Selling Price">
            <span style="color:#94a3b8;font-size:11px;">/</span>
            <input class="js-cart-discount pos-cart-inline-input" data-row="<?php echo e($r['row_id']); ?>" data-source="<?php echo e($r['discount_source']); ?>" value="<?php echo e((float)$r['discount_percent'] > 0 ? number_format((float)$r['discount_percent'],1,'.','') : ''); ?>" placeholder="0.0" type="number" step="0.1" min="0" <?php if($r['max_discount_percent'] > 0): ?> max="<?php echo e($r['max_discount_percent']); ?>" <?php endif; ?> style="width:50px;" title="Discount %">
            <span style="color:#94a3b8;font-size:11px;">%</span>
            <form method="POST" action="<?php echo e(route('mt.pos.remove')); ?>" class="js-pos-ajax-form js-cart-remove-form" style="margin:0;display:inline;">
              <?php echo csrf_field(); ?>
          ...
          <div class="pos-cart-row-meta">
            <div class="pos-cart-row-stats">
              <?php if($canSeeCost): ?>
                <span class="pos-cart-profit" data-row-profit="<?php echo e($r['row_id']); ?>">P:<?php echo e(number_format((float)$r['line_profit'],0)); ?></span>
              <?php endif; ?>
              <span class="pos-cart-line-total" data-row-total="<?php echo e($r['row_id']); ?>">Rs <?php echo e(number_format((float)$r['line_total'],0)); ?></span>
            </div>
          </div>
        </div>
      <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); if ($__empty_1): ?>
    ...
    <div class="pos-cart-checkout">
      <form method="POST" action="<?php echo e(route('mt.pos.checkout')); ?>" class="form-stack" id="posCheckoutForm">
        <?php echo csrf_field(); ?>
        ...
        <div class="pos-total-card">
          <div class="pos-total-caption">Bill Summary (<?php echo e(count($cartRows ?? [])); ?> items)</div>
          <div class="pos-total-row">
            <span>Gross Total</span>
            <strong id="cartGrossTotal"><?php echo e(number_format((float)($cartGrossTotal ?? 0),1)); ?></strong>
          </div>
          <div class="pos-total-row">
            <span>Subtotal</span>
            <strong id="cartSubtotal"><?php echo e(number_format((float)($cartSubtotal ?? 0),1)); ?></strong>
          </div>
          <div class="form-group pos-total-discount-group">
            ...
            <input id="posOverallDiscount" name="overall_discount" type="number" step="0.1" min="0" value="<?php echo e((float)($cartOverallDiscount ?? 0) > 0 ? number_format((float)($cartOverallDiscount ?? 0),1,'.','') : ''); ?>" placeholder="0.0" class="mt-input pos-total-input">
          </div>
          <div class="pos-total-row pos-total-final-row">
            <span>Net Total</span>
            <strong id="cartTotal" class="pos-total-amount">Rs <?php echo e(number_format((float)($cartTotal ?? 0),1)); ?></strong>
          </div>
          <?php if($canSeeCost): ?>
            <div class="pos-total-row pos-total-profit-row">
              <span>Profit Left</span>
              <strong id="cartProfit" class="pos-total-profit-value"><?php echo e(number_format((float)($cartProfit ?? 0),1)); ?></strong>
            </div>
          <?php endif; ?>
        ...
        <div class="pos-checkout-actions">
          ...
          <button type="button" onclick="reprintLastBill()" class="pos-action-btn pos-action-icon" id="reprintBtn" title="Reprint Last Bill" <?php if(empty(session('mt_last_sale_id'))): ?>disabled style="background:#555;border-color:#333;cursor:not-allowed;opacity:0.5;" <?php else: ?> style="background:#609;border-color:#407;" <?php endif; ?>>
...
(function(){
  /* ───── state ───── */
  const productsApiUrl = <?php echo json_encode(route('mt.pos.products'), 15, 512) ?>;
  const suggestUrl     = <?php echo json_encode(route('mt.pos.suggest'), 15, 512) ?>;
  const addUrl         = <?php echo json_encode(route('mt.pos.add'), 15, 512) ?>;
  const addCutUrl      = <?php echo json_encode(route('mt.pos.add_cut'), 15, 512) ?>;
  const leftoverAddFullUrl = <?php echo json_encode(route('mt.pos.leftover_add_full'), 15, 512) ?>;
  const leftoverAddCutUrl  = <?php echo json_encode(route('mt.pos.leftover_add_cut'), 15, 512) ?>;
  const removeUrl      = <?php echo json_encode(route('mt.pos.remove'), 15, 512) ?>;
  const token          = <?php echo json_encode(csrf_token(), 15, 512) ?>;
  const canSeeCost     = <?php echo json_encode($canSeeCost, 15, 512) ?>;

  let currentG = <?php echo json_encode($g, 15, 512) ?>;
  let currentC = <?php echo json_encode($c, 15, 512) ?>;
  let currentQ = <?php echo json_encode($q, 15, 512) ?>;
    ...
    if ((p.out_of_stock || p.stock_qty <= 0) && p.equivalents && p.equivalents.length > 0) {
      ...
      if (inStockEquivalents.length > 0) {
        ...
        inStockEquivalents.forEach(eq => {
          ...
          html += 'Add ' + esc(eq.company_name) + ' — ' + esc(eq.name) + ' (' + eq.stock_qty + ' in stock)';
          html += '</button>';
        });
        html += '</div></div>';
      }
  ...
  const updateItemUrl = <?php echo json_encode(route('mt.pos.update_item'), 15, 512) ?>;
  const updateOverallDiscountUrl = <?php echo json_encode(route('mt.pos.update_overall_discount'), 15, 512) ?>;
  ...
  let currentCartSummary = {
    gross_total: Number(<?php echo json_encode((float)($cartGrossTotal ?? 0), 15, 512) ?>) || 0,
    line_discount_total: Number(<?php echo json_encode((float)($cartLineDiscountTotal ?? 0), 15, 512) ?>) || 0,
    subtotal: Number(<?php echo json_encode((float)($cartSubtotal ?? 0), 15, 512) ?>) || 0,
    overall_discount: Number(<?php echo json_encode((float)($cartOverallDiscount ?? 0), 15, 512) ?>) || 0,
    total: Number(<?php echo json_encode((float)($cartTotal ?? 0), 15, 512) ?>) || 0,
    profit: Number(<?php echo json_encode((float)($cartProfit ?? 0), 15, 512) ?>) || 0
  };
...
async function posCheckout(action) {
  ...
  try {
    ...
    const successMsg = result.message || 'Sale completed';
    console.log(successMsg);

    // Handle printing
    if (result.should_print) {
      // Open sale page in new tab for printing
      const printUrl = result.sale_url + '?autoprint=1';
      const printWindow = window.open(printUrl, 'saleReceipt');
      if (printWindow) {
...
function reprintLastBill() {
  const lastSaleId = <?php echo json_encode(session('mt_last_sale_id') ?? null, 15, 512) ?>;
  if (!lastSaleId) {
  ...
  const printWindow = window.open(printUrl, 'reprintReceipt');
  if (printWindow) {
    printWindow.addEventListener('load', function() {
      setTimeout(() => printWindow.print(), 800);
    });
