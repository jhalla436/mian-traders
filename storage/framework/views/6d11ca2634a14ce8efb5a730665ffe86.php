  ...
  <div id="order-download-card" style="position: absolute; left: -9999px; top: -9999px; width: 800px; background: #ffffff; padding: 35px; font-family: 'Inter', system-ui, -apple-system, sans-serif; color: #1e293b;">
    ...
    <div style="display: flex; justify-content: space-between; align-items: flex-start; border-bottom: 2px solid #f1f5f9; padding-bottom: 20px; margin-bottom: 20px;">
      <div>
        ...
        <div style="font-size: 13px; color: #64748b; margin-top: 5px; font-weight: 500;">
          Order ID: <span style="font-weight: 700; color: #0f172a;">#<?php echo e($order->id); ?></span>
        </div>
      </div>
      <div style="text-align: right;">
        <h2 style="margin: 0; font-size: 20px; font-weight: 800; color: #0f172a;"><?php echo e($order->company?->name ?? 'Unknown Company'); ?></h2>
        <div style="font-size: 12px; color: #64748b; margin-top: 5px; font-weight: 500;">
          Order Date: <span style="font-weight: 700; color: #0f172a;"><?php echo e($order->order_date ? $order->order_date->format('Y-m-d') : now()->toDateString()); ?></span>
        </div>
      </div>
    </div>
    ...
    <div style="display: grid; grid-template-columns: repeat(4, 1fr); gap: 12px; margin-bottom: 25px;">
      <div style="background: #f8fafc; border: 1px solid #e2e8f0; border-radius: 10px; padding: 14px 10px; text-align: center;">
        ...
        <div style="font-size: 16px; font-weight: 900; color: #0f172a; margin-top: 4px;">Rs. <?php echo e(number_format((float)$order->goods_total, 2)); ?></div>
      </div>
      <div style="background: #f0fdf4; border: 1px solid #bbf7d0; border-radius: 10px; padding: 14px 10px; text-align: center;">
        ...
        <div style="font-size: 16px; font-weight: 900; color: #16a34a; margin-top: 4px;">Rs. <?php echo e(number_format((float)$order->paid_amount, 2)); ?></div>
      </div>
      <div style="background: #fef2f2; border: 1px solid #fecaca; border-radius: 10px; padding: 14px 10px; text-align: center;">
        ...
        <div style="font-size: 16px; font-weight: 900; color: #dc2626; margin-top: 4px;">Rs. <?php echo e(number_format((float)$order->balance, 2)); ?></div>
      </div>
      <div style="background: #f1f5f9; border: 1px solid #e2e8f0; border-radius: 10px; padding: 14px 10px; text-align: center; display: flex; flex-direction: column; justify-content: center; align-items: center;">
        ...
        <div style="font-size: 12px; font-weight: 800; color: #1e293b;"><?php echo e(strtoupper($order->payment_status ?: 'unpaid')); ?></div>
      </div>
    </div>
        ...
        <?php $__currentLoopData = $order->items; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $it): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
          ...
          <tr style="border-bottom: 1px solid #f1f5f9;">
            <td style="padding: 12px 10px; font-size: 12px; font-weight: 700; color: #0f172a;">
              <?php echo e($it->product?->name); ?>

              <?php if($it->product?->sku): ?>
                <div style="font-size: 10px; color: #64748b; font-weight: 400; margin-top: 2px;">SKU: <?php echo e($it->product->sku); ?></div>
              <?php endif; ?>
            </td>
            <td style="padding: 12px 10px; text-align: right; font-size: 12px; color: #475569;"><?php echo e(number_format($basePrice, 2)); ?></td>
            <td style="padding: 12px 10px; text-align: right; font-size: 12px; color: #475569;"><?php echo e($it->original_discount > 0 ? number_format($it->original_discount, 2) . '%' : '-'); ?></td>
            <td style="padding: 12px 10px; text-align: right; font-size: 12px; color: #475569;"><?php echo e($it->extra_discount > 0 ? number_format($it->extra_discount, 2) . '%' : '-'); ?></td>
            <td style="padding: 12px 10px; text-align: right; font-size: 12px; font-weight: 600; color: #0f172a;"><?php echo e(number_format((float)$it->unit_cost, 2)); ?></td>
            <td style="padding: 12px 10px; text-align: right; font-size: 12px; font-weight: 700; color: #0f172a;"><?php echo e(rtrim(rtrim(number_format((float)$it->qty, 3), '0'), '.')); ?></td>
            <td style="padding: 12px 10px; text-align: right; font-size: 12px; font-weight: 800; color: #4f46e5;"><?php echo e(number_format((float)$it->line_total, 2)); ?></td>
          </tr>
        <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
      </tbody>
    </table>
    ...
    <?php if($order->note): ?>
      <div style="background: #f8fafc; border: 1px solid #e2e8f0; border-radius: 10px; padding: 14px; font-size: 12px; color: #475569; margin-bottom: 25px;">
        <b style="color: #0f172a; margin-right: 4px;">Note:</b> <?php echo e($order->note); ?>
    ...
    <div style="border-top: 2px solid #f1f5f9; padding-top: 15px; text-align: center; font-size: 11px; color: #94a3b8; font-weight: 500;">
      Generated on <?php echo e(now()->toDateTimeString()); ?> | Mian Traders Management Software
    </div>
  </div>
      ...
      btn.addEventListener('click', function(e) {
        ...
        setTimeout(() => {
          html2canvas(card, {
            ...
            link.download = `Mian_Traders_Purchase_Order_#$<?php echo e($order->id); ?>_` + dateStr + '.png';
