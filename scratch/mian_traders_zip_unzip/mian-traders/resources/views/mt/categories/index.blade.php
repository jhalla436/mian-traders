...
@section('content')
  <div style="background:#fff;padding:14px;border-radius:10px;margin-bottom:12px;display:flex;justify-content:space-between;align-items:center;">
    ...
    <a href="{{ route('mt.categories.create') }}" style="padding:8px 12px;border-radius:8px;background:#16a34a;color:#fff;text-decoration:none;">Add Category</a>
  </div>
    ...
    <table style="width:100%;border-collapse:collapse;">
      ...
      <tbody>
        @forelse($categories as $c)
          <tr>
            <td style="border-bottom:1px solid #f3f4f6;padding:10px;">{{ $c->name }}</td>
            <td style="border-bottom:1px solid #f3f4f6;padding:10px;">
              {{ $c->group_key ?? '-' }}
            </td>
            <td style="border-bottom:1px solid #f3f4f6;padding:10px;">
              <a href="{{ route('mt.categories.edit', $c) }}" style="padding:6px 10px;border-radius:8px;background:#374151;color:#fff;text-decoration:none;">Edit</a>

              <form method="POST" action="{{ route('mt.categories.destroy', $c) }}" style="display:inline;">
                @csrf
                @method('DELETE')
