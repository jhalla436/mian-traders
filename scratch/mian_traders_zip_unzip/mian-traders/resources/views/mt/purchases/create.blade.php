...
@endphp
  ...
  <form method="POST" action="{{ route('mt.purchases.store') }}" style="background:#fff;padding:14px;border-radius:10px;">
    @csrf
    ...
    <div style="display:flex;gap:12px;flex-wrap:wrap;align-items:end;margin-bottom:12px;">
      <div>
        ...
        <input type="date" name="purchase_date" value="{{ old('purchase_date', now()->toDateString()) }}" required style="padding:10px;border:1px solid #ddd;border-radius:10px;">
        @error('purchase_date') <div style="color:#dc2626;font-size:12px;">{{ $message }}</div> @enderror
      </div>
      ...
      <div>
        ...
        <input name="supplier_name" value="{{ old('supplier_name') }}" placeholder="If not company" style="padding:10px;border:1px solid #ddd;border-radius:10px;min-width:240px;">
        @error('supplier_name') <div style="color:#dc2626;font-size:12px;">{{ $message }}</div> @enderror
      </div>

      <div>
        <div style="font-size:12px;color:#6b7280;">Invoice #</div>
        <input name="invoice_no" value="{{ old('invoice_no') }}" style="padding:10px;border:1px solid #ddd;border-radius:10px;min-width:180px;">
        @error('invoice_no') <div style="color:#dc2626;font-size:12px;">{{ $message }}</div> @enderror
      </div>
    </div>
    ...
    <div style="display:flex;gap:12px;flex-wrap:wrap;align-items:end;margin-bottom:12px;">
      <div>
        ...
        <input type="number" step="0.01" name="transport_charges" value="{{ old('transport_charges', 0) }}" style="padding:10px;border:1px solid #ddd;border-radius:10px;">
        @error('transport_charges') <div style="color:#dc2626;font-size:12px;">{{ $message }}</div> @enderror
      ...
      <div>
        ...
        <input type="number" step="0.01" name="payment_made" value="{{ old('payment_made', 0) }}" style="padding:10px;border:1px solid #ddd;border-radius:10px;">
        @error('payment_made') <div style="color:#dc2626;font-size:12px;">{{ $message }}</div> @enderror
      </div>
