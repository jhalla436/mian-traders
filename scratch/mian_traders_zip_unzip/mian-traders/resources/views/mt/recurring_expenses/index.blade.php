...
@section('content')
  ...
  <div style="background:#fff;padding:14px;border-radius:10px;margin-bottom:12px;">
    ...
    <form method="POST" action="{{ route('mt.recurring_expenses.store') }}" style="display:flex;gap:10px;flex-wrap:wrap;align-items:end;">
      @csrf
    ...
    @if($errors->any())
      <div style="margin-top:10px;color:#dc2626;font-size:12px;">{{ implode(' | ', $errors->all()) }}</div>
    @endif
  </div>
      ...
      <tbody>
        @forelse($rows as $r)
          <tr>
            ...
            <td style="border-bottom:1px solid #f2f2f2;padding:8px;">{{ $r->day_of_month }}</td>
            ...
            <td style="border-bottom:1px solid #f2f2f2;padding:8px;">
              ...
              @endif
              <div style="font-size:12px;color:#6b7280;">Last: {{ $r->last_generated_month ?? '-' }}</div>
            </td>
            <td style="border-bottom:1px solid #f2f2f2;padding:8px;">
              <form method="POST" action="{{ route('mt.recurring_expenses.update', $r) }}" style="display:inline;">
                @csrf
                @method('PUT')
              ...
              <form method="POST" action="{{ route('mt.recurring_expenses.destroy', $r) }}" style="display:inline;" onsubmit="return confirm('Delete this recurring expense?')">
                @csrf
                @method('DELETE')
