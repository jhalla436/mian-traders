...
@section('content')
  <div style="background:#fff;padding:14px;border-radius:10px;margin-bottom:12px;display:flex;justify-content:space-between;align-items:center;gap:10px;flex-wrap:wrap;">
    ...
    <form method="GET" style="display:flex;gap:8px;align-items:center;">
      <input name="q" value="{{ $q ?? '' }}" placeholder="Search name/key..."
  ...
  <div style="background:#fff;padding:14px;border-radius:10px;margin-bottom:12px;">
    <a href="{{ route('mt.company_groups.create') }}"
  ...
  <div style="background:#fff;padding:14px;border-radius:10px;">
    <table style="width:100%;border-collapse:collapse;">
      ...
      <tbody>
        @forelse($rows as $r)
          <tr>
            <td style="border-bottom:1px solid #f2f2f2;padding:8px;font-weight:800;">{{ $r->name }}</td>
            <td style="border-bottom:1px solid #f2f2f2;padding:8px;">{{ $r->key }}</td>
            <td style="border-bottom:1px solid #f2f2f2;padding:8px;text-align:right;">{{ (int)$r->sort_order }}</td>
            <td style="border-bottom:1px solid #f2f2f2;padding:8px;">
              {{ (int)$r->is_active === 1 ? 'Active' : 'Disabled' }}
            </td>
            <td style="border-bottom:1px solid #f2f2f2;padding:8px;">
              <a href="{{ route('mt.company_groups.edit', $r) }}"
                 style="padding:6px 10px;border-radius:8px;background:#374151;color:#fff;text-decoration:none;">Edit</a>

              <form method="POST" action="{{ route('mt.company_groups.destroy', $r) }}"
