...
?>
  ...
  <div style="background:#fff;padding:14px;border-radius:10px;margin-bottom:12px;display:flex;justify-content:space-between;align-items:center;gap:10px;flex-wrap:wrap;">
    ...
    <a href="<?php echo e(route('mt.company_orders.index')); ?>" style="padding:10px 12px;border-radius:10px;background:#374151;color:#fff;text-decoration:none;">Back</a>
  </div>

  <form method="POST" action="<?php echo e(route('mt.company_orders.store')); ?>" style="background:#fff;padding:14px;border-radius:10px;">
    <?php echo csrf_field(); ?>
    ...
    <div style="display:flex;gap:12px;flex-wrap:wrap;align-items:end;margin-bottom:12px;">
      <div>
        ...
        <input type="date" name="order_date" value="<?php echo e(old('order_date', now()->toDateString())); ?>" required style="padding:10px;border:1px solid #ddd;border-radius:10px;">
...
unset($__errorArgs, $__bag); ?>
  ...
  <script>
    ...
    async function fetchCost(companyId, productId){
      const url = `<?php echo e(route('mt.company_orders.calc_cost')); ?>?company_id=${encodeURIComponent(companyId)}&product_id=${encodeURIComponent(productId)}`;
...
<?php echo $__env->make('mt.layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\mian-traders\resources\views/mt/company_orders/create.blade.php ENDPATH**/ ?>