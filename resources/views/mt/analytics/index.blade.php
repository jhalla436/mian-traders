...
<div class="page-header">
    ...
    <div class="page-actions">
        <form action="{{ route('mt.analytics.index') }}" method="GET" style="display: flex; gap: 8px; align-items: center;">
...
<div class="analytics-grid">
                ...
                <div class="analytics-item">
                    ...
                    <div class="item-info">
                        ...
                        <div class="item-stats">
                            <span>Qty: {{ number_format($item->total_qty, 0) }}</span>
                            <span>•</span>
                            <span>Avg Price: {{ number_format($item->total_revenue / ($item->total_qty ?: 1), 0) }}</span>
                        </div>
                        <div class="progress-bar-container">
                            <div class="progress-bar progress-blue" style="width: {{ ($item->total_revenue / $maxRevenue) * 100 }}%"></div>
                        </div>
                    </div>
                    <div class="item-value">{{ number_format($item->total_revenue, 0) }}</div>
                </div>
            @endforeach
            ...
            @foreach($topCategories as $index => $item)
                <div class="analytics-item">
                    ...
                    <div class="item-info">
                        ...
                        <div class="item-stats">
                            <span>Units Sold: {{ number_format($item->total_qty, 0) }}</span>
                        </div>
                        <div class="progress-bar-container">
                            <div class="progress-bar progress-purple" style="width: {{ ($item->total_revenue / $maxCatRevenue) * 100 }}%"></div>
                        </div>
                    </div>
                    <div class="item-value">{{ number_format($item->total_revenue, 0) }}</div>
                </div>
    ...
    <div class="analytics-card">
        ...
        <div class="analytics-list">
            @php $maxCompRevenue = $topCompanies->max('total_revenue') ?: 1; @endphp
            @foreach($topCompanies as $index => $item)
                <div class="analytics-item">
                    <div class="item-rank top-{{ $index + 1 }}">{{ $index + 1 }}</div>
                    <div class="item-info">
                        <div class="item-name">{{ $item->name }}</div>
                        <div class="item-stats">
                            <span>Contribution: {{ number_format(($item->total_revenue / $maxCompRevenue) * 100, 1) }}% of top</span>
                        </div>
                        <div class="progress-bar-container">
                            <div class="progress-bar progress-teal" style="width: {{ ($item->total_revenue / $maxCompRevenue) * 100 }}%"></div>
                        </div>
                    </div>
                    <div class="item-value">{{ number_format($item->total_revenue, 0) }}</div>
                </div>
    ...
    {{-- Sales Trend Placeholder / Table --}}
    <div class="analytics-card">
        <div class="analytics-card-header">
            <h3 class="analytics-card-title">Daily Sales Trend</h3>
            <span class="badge badge-primary">Last {{ $days }} Days</span>
        </div>
        <div style="padding: 0;">
            <table class="mt-table" style="margin-top: 0;">
                ...
                <tbody>
                    @foreach($salesTrend->reverse()->take(10) as $trend)
                        <tr>
                            <td style="padding-left: 24px; font-weight: 600;">{{ \Carbon\Carbon::parse($trend->date)->format('D, M j') }}</td>
                            <td style="text-align: right; padding-right: 24px; font-weight: 700; color: #16a34a;">{{ number_format($trend->total_revenue, 0) }}</td>
                        </tr>
                    @endforeach
                </tbody>
            </table>
