...
@section('content')
  <div class="page-header">
    <h2 class="page-title">
      Sizes for: <strong>{{ $category->name }}</strong>
      @if($category->company)
        <span class="text-muted">({{ $category->company->name }})</span>
      @endif
    </h2>
  ...
  <div class="card">
    ...
    @if($sizes->count() > 0)
      <div style="display: flex; flex-wrap: wrap; gap: 10px; margin-bottom: 16px;">
        <form method="POST" action="{{ route('mt.categories.sizes.destroy_all', $category) }}" style="display:inline-block;">
          @csrf
          @method('DELETE')
    ...
    <!-- Single Size Form -->
    <form id="single-form" method="POST" action="{{ route('mt.categories.sizes.store', $category) }}" class="form-grid" style="display: block;">
      @csrf
      ...
      <div class="form-group" style="grid-column: 1 / -1;">
        ...
        @error('size_type') <div class="form-error">{{ $message }}</div> @enderror
      </div>
    ...
    <!-- Bulk Add Form -->
    <form id="bulk-form" method="POST" action="{{ route('mt.categories.sizes.store', $category) }}" style="display: none;">
      @csrf
      ...
      <div class="form-group" style="grid-column: 1 / -1;">
        ...
        ></textarea>
        @error('bulk_sizes') <div class="form-error">{{ $message }}</div> @enderror
      </div>
  ...
  <div class="card" style="margin-top: 24px;">
    ...
    <div style="display:flex; justify-content:space-between; align-items:center; gap:12px; flex-wrap:wrap; margin-bottom:16px;">
      <h3 style="margin-top: 0;">Sizes ({{ $sizes->count() }})</h3>
      @if($sizes->count() > 0)
        <div>
    ...
    @if($sizes->count() > 0)
      <form id="delete-selected-category-sizes-form" method="POST" action="{{ route('mt.categories.sizes.destroy_selected', $category) }}">
        @csrf
        @method('DELETE')
          ...
          <table class="mt-table">
            ...
            <tbody>
              @foreach($sizes as $size)
                <tr>
                  <td><input class="size-row-check" type="checkbox" name="size_ids[]" value="{{ $size->id }}" /></td>
                  <td>{{ $size->length_in }}"</td>
                  <td>{{ $size->width_in }}"</td>
                  <td>{{ $size->height_in }}"</td>
                  <td>
                    @if($size->size_type === 'uncovered_sofa')
                      Uncovered Sofa Seats
                    @elseif($size->size_type === 'uncovered_slab')
                      Uncovered Slab Sheets
                    @else
                      Standard
                    @endif
                  </td>
                  <td class="font-bold">{{ $size->getDisplayName() }}</td>
                  <td>
                    <form method="POST" action="{{ route('mt.categories.sizes.destroy', [$category, $size]) }}" style="display: inline;">
                      @csrf @method('DELETE')
