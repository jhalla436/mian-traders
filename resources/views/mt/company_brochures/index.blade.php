    ...
    @foreach($companies as $company)
      ...
      <div class="company-card">
        ...
        <div class="company-card-body">
          @if($company->brochures->isEmpty())
            <div class="empty-state">
              No brochures uploaded yet
            </div>
          @else
            <ul class="brochures-list">
              @foreach($company->brochures as $index => $brochure)
                @php
                ...
                <li class="brochure-item">
                  <div class="brochure-info">
                    <div class="brochure-icon {{ $isPdf ? 'pdf' : 'image' }}">
                      @if($isPdf)
                        <!-- PDF Icon -->
                    ...
                    <div class="brochure-details">
                      <span class="brochure-name" title="{{ $brochure->file_name }}">{{ $brochure->file_name }}</span>
                      <span class="brochure-date">Uploaded: {{ $brochure->created_at->format('M d, Y') }}</span>
                    </div>
                  </div>
                  
                  <div class="brochure-actions">
                    <button type="button" class="btn btn-secondary btn-xs"
                            data-brochures="{!! htmlspecialchars($brochuresJson, ENT_QUOTES, 'UTF-8') !!}"
                            data-index="{{ $index }}"
                            onclick="openViewer(this)">
                      View
                    </button>
                    <a href="{{ route('mt.company_brochures.file', $brochure) }}" download="{{ $brochure->file_name }}" class="btn btn-secondary btn-xs" style="text-decoration: none; display: inline-flex; align-items: center; justify-content: center;">
                      Download
                    </a>
                    <form method="POST" action="{{ route('mt.company_brochures.destroy', $brochure) }}" onsubmit="return confirm('Are you sure you want to delete this brochure?')">
                      @csrf @method('DELETE')
          ...
          <div class="upload-form-section">
            ...
            <form method="POST" action="{{ route('mt.company_brochures.store') }}" enctype="multipart/form-data">
              @csrf
