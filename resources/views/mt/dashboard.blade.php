...
<style>
  ...
  .metric-card.orange .accent-bar { background: linear-gradient(90deg, #f59e0b, #fbbf24); }
  ...
  .sales-table td {
    padding: 14px 24px; border-top: 1px solid #f8fafc; font-size: 13px; color: #334155;
...
<div class="dash-hero">
  <div style="display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:16px;">
    ...
    <div class="dash-hero-actions">
      <a href="{{ route('mt.pos.index') }}" class="dash-hero-btn primary">
      ...
      <a href="{{ route('mt.sales.index') }}" class="dash-hero-btn ghost">
      ...
      <a href="{{ route('mt.udhar.index') }}" class="dash-hero-btn danger">
...
<div class="metrics-row">
  <div class="metric-card red">
    ...
    <div class="metric-value">{{ number_format((float)($totalUdharAll ?? 0), 0) }}</div>
    <div class="metric-footer">
      <span class="metric-sub">All customers</span>
      <a href="{{ route('mt.udhar.index') }}" class="metric-link">View <span>→</span></a>
    </div>
  </div>
  ...
  <div class="metric-card blue">
    ...
    <div class="metric-value">{{ number_format((float)($salesToday ?? 0), 0) }}</div>
    <div class="metric-footer">
      <span class="metric-sub">{{ now()->format('D, M j') }}</span>
      @if($canSeeCost)
        <span class="metric-sub" style="color:#16a34a;font-weight:700;">Profit: {{ number_format((float)($profitToday ?? 0), 0) }}</span>
      @endif
    </div>
  </div>
  ...
  <div class="metric-card purple">
    ...
    <div class="metric-value">{{ number_format((float)($salesThisWeek ?? 0), 0) }}</div>
    <div class="metric-footer">
      <span class="metric-sub">Mon – Sun</span>
      @if($canSeeCost)
        <span class="metric-sub" style="color:#16a34a;font-weight:700;">Profit: {{ number_format((float)($profitThisWeek ?? 0), 0) }}</span>
      @endif
    </div>
  </div>
  ...
  <div class="metric-card teal">
    ...
    <div class="metric-value">{{ number_format((float)($salesThisMonth ?? 0), 0) }}</div>
    <div class="metric-footer">
      <span class="metric-sub">{{ now()->format('F Y') }}</span>
      @if($canSeeCost)
        <span class="metric-sub" style="color:#16a34a;font-weight:700;">Profit: {{ number_format((float)($profitThisMonth ?? 0), 0) }}</span>
      @endif
    </div>
  </div>
</div>
...
<div class="dash-grid">
  ...
  <div class="dash-card">
    <div class="dash-card-header">
      ...
      <a href="{{ route('mt.sales.index') }}" class="dash-card-badge" style="background:#eef2ff;color:#6366f1;text-decoration:none;">View All →</a>
    </div>
        ...
        <tbody>
          @foreach($recentSales as $s)
            <tr>
              ...
              <td>
                <div class="customer-name">{{ $s->customer_name ?: 'Walk-in' }}</div>
                @if($s->customer_phone)
                  <div class="customer-phone">{{ $s->customer_phone }}</div>
                @endif
              </td>
              <td class="right amount">{{ number_format((float)$s->total_amount, 0) }}</td>
              <td class="right amount">{{ number_format((float)$s->paid_amount, 0) }}</td>
              <td class="right {{ (float)$s->balance_amount > 0 ? 'balance-red' : 'balance-green' }}">
                {{ number_format((float)$s->balance_amount, 0) }}
              </td>
              <td>
                @php $st = strtolower($s->status ?? 'paid'); @endphp
                <span class="status-badge status-{{ $st }}">{{ $st }}</span>
              </td>
              <td><a href="{{ route('mt.sales.show', $s) }}" class="view-btn">View</a></td>
            </tr>
          @endforeach
        </tbody>
      </table>
  ...
  <div class="side-stack">
    ...
    <div class="alert-card stock-alert">
      ...
      <div class="alert-value">{{ (int)($lowStockCount ?? 0) }}</div>
      <a href="{{ route('mt.products.index') }}" class="alert-link">View Products <span>→</span></a>
    </div>

    @if($canSeeCost)
    {{-- Profit Summary --}}
    <div class="dash-card" style="padding:24px;">
      <div class="dash-card-title" style="margin-bottom:16px;">Profit Summary</div>
      <div style="display:flex;flex-direction:column;gap:12px;">
        <div style="display:flex;justify-content:space-between;align-items:center;">
          <span style="font-size:13px;color:#64748b;font-weight:500;">Today</span>
          <span style="font-size:15px;font-weight:800;color:#16a34a;">{{ number_format((float)($profitToday ?? 0), 0) }}</span>
        </div>
        <div style="background:#f1f5f9;height:1px;"></div>
        <div style="display:flex;justify-content:space-between;align-items:center;">
          <span style="font-size:13px;color:#64748b;font-weight:500;">This Week</span>
          <span style="font-size:15px;font-weight:800;color:#16a34a;">{{ number_format((float)($profitThisWeek ?? 0), 0) }}</span>
        </div>
        <div style="background:#f1f5f9;height:1px;"></div>
        <div style="display:flex;justify-content:space-between;align-items:center;">
          <span style="font-size:13px;color:#64748b;font-weight:500;">This Month</span>
          <span style="font-size:15px;font-weight:800;color:#16a34a;">{{ number_format((float)($profitThisMonth ?? 0), 0) }}</span>
        </div>
      </div>
    </div>
    @endif
