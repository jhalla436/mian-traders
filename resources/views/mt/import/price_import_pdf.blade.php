...
@section('title', 'PDF/CSV Price Import')
...
<div class="page-header">
  ...
  <a href="{{ route('mt.products.index') }}" class="btn btn-secondary btn-sm">← Back to Products</a>
</div>

{{-- Navigation Tabs --}}
<div class="mt-tabs" style="margin-bottom: 20px;">
  <a href="{{ route('mt.products.price_import_form') }}" class="mt-tab">CSV / Text Import</a>
  <a href="{{ route('mt.products.price_import_pdf_form') }}" class="mt-tab active">PDF Price List Parser</a>
</div>
...
<div class="card" style="max-width:100%; margin-bottom: 20px;">
  <div class="card-body">
    ...
    <form method="POST" action="{{ route('mt.products.price_import_pdf_process') }}" enctype="multipart/form-data" class="form-stack">
      @csrf
        ...
        <div class="form-group">
          ...
          <select name="default_company_id" class="mt-select" required>
            ...
            @foreach($companies ?? [] as $c)
              <option value="{{ $c->id }}" @selected(isset($selectedCompanyId) && (int)$selectedCompanyId === (int)$c->id)>{{ $c->name }}</option>
            @endforeach
          </select>
        </div>
        ...
        <div class="form-group">
          ...
          <select name="default_category_id" class="mt-select" required>
            ...
            @foreach($categories ?? [] as $cat)
              <option value="{{ $cat->id }}" @selected(isset($selectedCategoryId) && (int)$selectedCategoryId === (int)$cat->id)>{{ $cat->name }}</option>
            @endforeach
          </select>
        </div>
      </div>
...
@if(isset($parsedItems))
  <div class="card" style="max-width:100%; border: 1.5px solid #6366f1;">
    <div class="card-body">
      ...
      <form id="confirmImportForm" method="POST" action="{{ route('mt.products.price_import_pdf_confirm') }}">
        @csrf
          ...
          <table class="mt-table" style="width:100%; margin:0;">
            <thead style="position: sticky; top:0; z-index:10; background:#f8fafc; box-shadow: 0 1px 0 #e2e8f0;">
              <tr>
                ...
                <th style="font-size: 10px; padding: 10px 14px; width: 110px; text-align:right;">Purchase Price (Rs)</th>
              </tr>
            </thead>
            <tbody>
              @forelse($parsedItems as $idx => $item)
                <tr>
                  <td style="text-align:center; padding: 8px 14px; vertical-align:middle;">
                    <input type="checkbox" name="items[{{ $idx }}][import]" value="1" checked class="row-checkbox" style="width:16px; height:16px; cursor:pointer;" />
                  </td>
                  ...
                  <td style="padding: 8px 14px;">
                    <input type="text" name="items[{{ $idx }}][name]" value="{{ $item['name'] }}" class="mt-input" style="padding:4px 8px; font-size:12px; min-height:28px;" required />
                  </td>
                  <td style="padding: 8px 14px;">
                    <input type="number" step="0.1" name="items[{{ $idx }}][length_in]" value="{{ $item['length_in'] }}" class="mt-input" style="padding:4px 8px; font-size:12px; min-height:28px; text-align:right;" required />
                  </td>
                  <td style="padding: 8px 14px;">
                    <input type="number" step="0.1" name="items[{{ $idx }}][width_in]" value="{{ $item['width_in'] }}" class="mt-input" style="padding:4px 8px; font-size:12px; min-height:28px; text-align:right;" required />
                  </td>
                  <td style="padding: 8px 14px;">
                    <input type="number" step="0.1" name="items[{{ $idx }}][height_in]" value="{{ $item['height_in'] }}" class="mt-input" style="padding:4px 8px; font-size:12px; min-height:28px; text-align:right;" />
                  </td>
                  <td style="padding: 8px 14px;">
                    <input type="number" step="0.01" name="items[{{ $idx }}][price]" value="{{ $item['price'] }}" class="mt-input" style="padding:4px 8px; font-size:12px; min-height:28px; text-align:right; font-weight:700; color:#0f172a;" required />
                  </td>
                  <td style="padding: 8px 14px;">
                    <input type="number" step="0.01" name="items[{{ $idx }}][purchase_price]" value="{{ $item['purchase_price'] ?? '' }}" class="mt-input" style="padding:4px 8px; font-size:12px; min-height:28px; text-align:right; font-weight:700; color:#166534;" readonly />
                  </td>
                </tr>
              @empty
                <tr>
        ...
        @if(count($parsedItems) > 0)
          <div style="display:flex; justify-content:space-between; align-items:center; flex-wrap:wrap; gap:12px;">
            <div style="font-size:13px; color:#475569; font-weight:500;">
              Total parsed items: <strong style="color:#4f46e5;" id="checkedCount">{{ count($parsedItems) }}</strong> / {{ count($parsedItems) }} selected.
            </div>
    ...
    <div class="card-body">
      ...
      <div style="display:grid; grid-template-columns:repeat(auto-fit, minmax(180px, 1fr)); gap:12px; margin-bottom:20px;">
        <div style="background:#f0fdf4; border:1px solid #bbf7d0; padding:12px; border-radius:10px; text-align:center;">
          ...
          <div style="font-size:24px; font-weight:900; color:#166534; margin-top:4px;">{{ $report['total'] }}</div>
        </div>
        <div style="background:#f0fdf4; border:1px solid #bbf7d0; padding:12px; border-radius:10px; text-align:center;">
          <div style="font-size:11px; color:#15803d; font-weight:700; text-transform:uppercase;">Products Auto-Created</div>
          <div style="font-size:24px; font-weight:900; color:#166534; margin-top:4px;">{{ $report['created_products'] }}</div>
        </div>
        <div style="background:#f0fdf4; border:1px solid #bbf7d0; padding:12px; border-radius:10px; text-align:center;">
          <div style="font-size:11px; color:#15803d; font-weight:700; text-transform:uppercase;">Shop Prices Created</div>
          <div style="font-size:24px; font-weight:900; color:#166534; margin-top:4px;">{{ $report['created_shop_prices'] }}</div>
        </div>
        <div style="background:#f0fdf4; border:1px solid #bbf7d0; padding:12px; border-radius:10px; text-align:center;">
          <div style="font-size:11px; color:#15803d; font-weight:700; text-transform:uppercase;">Shop Prices Updated</div>
          <div style="font-size:24px; font-weight:900; color:#166534; margin-top:4px;">{{ $report['updated_shop_prices'] }}</div>
        </div>
      </div>
          ...
          <table class="mt-table" style="width:100%; margin:0;">
            ...
            <tbody>
              @foreach($report['failed'] as $f)
                <tr>
                  <td style="color:#991b1b; font-weight:700;">{{ $f['row'] }}</td>
                  <td style="color:#7f1d1d;">{{ $f['reason'] }}</td>
                </tr>
              @endforeach
            </tbody>
          </table>
