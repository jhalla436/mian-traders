...
<div class="mt-tabs" style="margin-bottom: 20px;">
  <a href="{{ route('mt.products.price_import_form') }}" class="mt-tab active">CSV / Text Import</a>
  <a href="{{ route('mt.products.price_import_pdf_form') }}" class="mt-tab">PDF Price List Parser</a>
</div>
...
<div class="card" style="max-width:860px;">
  <div class="card-body">
    ...
    <form method="POST" action="{{ route('mt.products.price_import_process') }}" enctype="multipart/form-data">
      @csrf
      ...
      <div class="form-group">
        ...
        @error('files') <div class="form-error">{{ $message }}</div> @enderror
        @error('files.*') <div class="form-error">{{ $message }}</div> @enderror
      </div>
    ...
    @if(isset($report))
      <hr>
      <h4>Report @if(!empty($dry_run)) (dry run) @endif</h4>
      <ul>
        <li>Total rows: {{ $report['total'] }}</li>
        <li>Products auto-created: {{ $report['created_products'] }}</li>
        <li>Shop prices created: {{ $report['created_shop_prices'] }}</li>
        <li>Shop prices updated: {{ $report['updated_shop_prices'] }}</li>
      </ul>
