...
<body>
    ...
    <div class="header">
        ...
        <div class="header-meta">Order ID: #{{ $order->id }}</div>
    </div>
    ...
    <table class="meta-table">
        ...
        <tr>
            ...
            <td>{{ $order->company->phone_main }}</td>
    ...
    <table class="items-table">
        ...
        <tbody>
            @foreach($order->items as $index => $it)
                ...
                <tr>
                    <td>{{ $index + 1 }}</td>
                    <td style="color:#64748b; font-size:11px;">{{ $it->product?->category ? $it->product->category->name : 'Uncategorized' }}</td>
                    <td>
                        <strong>{{ $it->product?->name }}</strong>
                        @if($it->product?->sku)
                            <div style="font-size:10px; color:#94a3b8;">SKU: {{ $it->product->sku }}</div>
                        @endif
                        @if($isShell)
                            <div style="font-size:10px; color:#6366f1;">Shell Mode: ({{ $it->product->width_in }}x{{ $it->product->length_in }}x{{ $it->product->height_in }})</div>
                        @endif
                    </td>
                    <td class="text-right">{{ number_format($basePrice, 2) }}</td>
                    <td class="text-right">{{ $it->original_discount > 0 ? number_format($it->original_discount, 1) . '%' : '-' }}</td>
                    <td class="text-right">{{ $it->extra_discount > 0 ? number_format($it->extra_discount, 1) . '%' : '-' }}</td>
    ...
    <table class="total-section">
        <tr>
            ...
            <td class="text-right"><strong>Rs. {{ number_format((float)$order->goods_total, 2) }}</strong></td>
        </tr>
        <tr>
            <td class="total-label">Paid Amount:</td>
            <td class="text-right">Rs. {{ number_format((float)$order->paid_amount, 2) }}</td>
        </tr>
        <tr class="grand-total">
            <td class="total-label" style="color:#4f46e5;">Remaining Balance:</td>
            <td class="text-right">Rs. {{ number_format((float)$order->balance, 2) }}</td>
        </tr>
    </table>
