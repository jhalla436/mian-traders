...
<div class="page-header">
  ...
  <div class="page-actions">
    <form method="GET" class="flex gap-8 items-center">
      <input name="q" value="{{ $q }}" placeholder="Search name/sku/id..." class="mt-input" style="width:220px;">
...
@if(session('import_errors'))
  <div class="errors-box" style="margin-bottom: 20px; border: 1.5px solid #fca5a5; background: #fff5f5; border-radius: 12px; padding: 16px;">
    <h4 style="color: #991b1b; font-weight: 700; margin-bottom: 8px; display: flex; align-items: center; gap: 6px;">
      ...
      Import Validation Warnings ({{ count(session('import_errors')) }})
    </h4>
...
<div class="page-actions mb-16" style="display:flex;gap:8px;align-items:center;flex-wrap:wrap;">
  <a href="{{ route('mt.products.create') }}" class="btn btn-success">+ Add Product</a>
  <a href="{{ route('mt.products.import_form') }}" class="btn btn-primary">Import Products</a>
  <a href="{{ route('mt.products.price_import_pdf_form') }}" class="btn btn-teal">Import PDF Price Sheet</a>
  <form id="bulk-delete-form" method="POST" action="{{ route('mt.products.bulk_destroy') }}" style="display:inline-block;margin-left:8px;">
    @csrf
  ...
  <form id="delete-by-keyword-form" method="POST" action="{{ route('mt.products.bulk_destroy_by_keyword') }}" style="display:none;">
    @csrf
    <input type="hidden" name="keyword" id="delete-keyword-input">
  </form>
  <form id="bulk-clear-sizes-form" method="POST" action="{{ route('mt.products.bulk_clear_sizes') }}" style="display:inline-block;margin-left:8px;">
    @csrf
...
<div class="table-card" style="overflow-x:auto;padding:8px;">
  <table class="mt-table" style="font-size:13px;">
    ...
    <tbody>
      @forelse($productGroups as $companyIndex => $companyGroup)
        @php
          $isOpen = ($q !== '');
        ...
        <tr class="bg-slate-50" style="background:#f8fafc;">
          @if(auth()->user()?->role !== 'cashier')
            <td></td>
          @endif
          <td colspan="{{ $headerColspan }}" style="padding:10px 8px;font-weight:700; display:flex; justify-content:space-between; align-items:center; border-bottom:1px solid #e2e8f0;">
            <button type="button" class="btn btn-secondary btn-xs js-toggle-company" data-company-key="{{ $companyKey }}" data-open="{{ $dataOpenVal }}" style="margin:0;">{{ $btnSymbol }} {{ $companyGroup['name'] }} ({{ $companyFamilyCount }} main products, {{ $companyVariantCount }} variants)</button>
            @if(auth()->user()?->role !== 'cashier')
              <button type="button" class="btn btn-danger btn-xs js-delete-company-btn" data-ids="{{ $companyProductIds }}" data-company-name="{{ $companyGroup['name'] }}" style="padding:2px 6px; font-size:11px; margin-right:8px;">Delete Company</button>
            @endif
          </td>
        </tr>

        @foreach($companyGroup['families'] as $familyName => $familyProducts)
          @php
          ...
          <tr class="js-company-family-row {{ $companyKey }}" style="{{ $displayStyle }}background:#fff;">
            @if(auth()->user()?->role !== 'cashier')
              <td style="padding-left:18px;">
                <input class="family-check" type="checkbox" data-family-key="{{ $familyKey }}" />
              </td>
            @endif
            <td colspan="{{ $headerColspan }}" style="padding:8px 8px 8px 18px;font-weight:600; display:flex; justify-content:space-between; align-items:center; border-bottom:1px solid #f1f5f9;">
              <button type="button" class="btn btn-outline btn-xs js-toggle-family" data-company-key="{{ $companyKey }}" data-family-key="{{ $familyKey }}" data-open="{{ $dataOpenVal }}" style="margin:0;">{{ $btnSymbol }} {{ $familyName }} ({{ $variantCount }} variants)</button>
              @if(auth()->user()?->role !== 'cashier')
                <button type="button" class="btn btn-danger btn-xs js-delete-family-btn" data-ids="{{ $familyProductIds }}" data-family-name="{{ $familyName }}" style="padding:2px 6px; font-size:11px; margin-right:8px;">Delete Category</button>
              @endif
            </td>
          </tr>
          ...
          @foreach($familyProducts as $p)
            @php
              ...
              if ($purchase <= 0 && $p instanceof \App\Models\Product) {
                $purchase = \App\Services\PricingService::purchasePrice($p);
              }
            @endphp

            <tr class="js-family-variant-row {{ $familyKey }}" style="{{ $displayStyle }}">
              @if(auth()->user()?->role !== 'cashier')
                <td><input class="row-check" data-family-key="{{ $familyKey }}" type="checkbox" value="{{ $p->id }}" /></td>
              @endif
              @php
              ...
              <td class="font-bold" style="padding-left: 24px; color: #475569;">
                <span style="color:#94a3b8; font-size:11px; margin-right:4px;">↳</span> {{ $displayName }}
              </td>
              <td>{{ $p->category?->name ?? '-' }}</td>
              <td>{{ $p->company?->name ?? '-' }}</td>
              <td class="text-right">{{ number_format((float)$p->mrp,2) }}</td>
              @php
              ...
              @endphp
              <td class="text-right inline-editable" data-field="max_discount_percent" data-product-id="{{ $p->id }}" title="Click to edit">
                <span class="display-value">{{ $effectiveDiscount > 0 ? number_format($effectiveDiscount,2) . '%' : 'No limit' }}</span>
                @if($usingCompanyDefault)
                  <div class="company-default-badge">company default</div>
                @endif
                <input type="number" class="edit-input" style="width:60px;display:none;" min="0" max="100" step="0.01" value="{{ $productDiscountValue }}">
              </td>
              @if($canSeeCost)
                <td class="text-right font-bold">{{ number_format((float)$purchase,2) }}</td>
              @endif
              @php
                $shopBreakdown = [];
                if (isset($p->shop_products) && is_array($p->shop_products)) {
                    $shopBreakdown = $p->shop_products;
                } elseif ($p instanceof \App\Models\Product && $p->relationLoaded('shopProducts')) {
                    $shopBreakdown = $p->shopProducts->map(function($sp) {
                        return [
              ...
              @endphp
              @if(\App\Support\ShopContext::isAll())
                <td class="text-right" style="color: #1e293b; padding: 8px 12px; vertical-align: top;">
                  <span class="font-bold">{{ number_format((float)$p->stock_qty, 2) }}</span>
                  @php
                    $hasStockBreakdown = collect($shopBreakdown)->filter(fn($spItem) => (float)$spItem['stock_qty'] > 0)->isNotEmpty();
                  @endphp
                  @if($hasStockBreakdown)
                    <div style="font-size: 11px; color: #64748b; margin-top: 4px; line-height: 1.4; text-align: right;">
                      @foreach($shopBreakdown as $spItem)
                        @if((float)$spItem['stock_qty'] > 0)
                          <div style="white-space: nowrap;">
                            <span style="color:#cbd5e1; font-size:10px;">↳</span> 
                            <span style="font-weight: 500;">{{ $spItem['shop_name'] }}:</span> {{ number_format((float)$spItem['stock_qty'], 2) }}
                          </div>
                        @endif
                      @endforeach
                    </div>
              ...
              @else
                <td class="text-right inline-editable" data-field="stock_qty" data-product-id="{{ $p->id }}" title="Click to edit">
                  <span class="display-value">{{ number_format((float)$p->stock_qty,2) }}</span>
                  <input type="number" class="edit-input" style="width:70px;display:none;" min="0" step="0.01" value="{{ $p->stock_qty ?? 0 }}">
                </td>
              @endif
              <td>
                <div class="actions">
                  @if(auth()->user()?->role !== 'cashier')
                    <a href="{{ route('mt.products.edit', $p->id) }}" class="btn btn-secondary btn-xs">Edit</a>
                  @else
        ...
        @endforeach
      @empty
        <tr class="empty-row"><td colspan="{{ $canSeeCost ? (auth()->user()?->role !== 'cashier' ? 9 : 8) : (auth()->user()?->role !== 'cashier' ? 8 : 7) }}">No products found.</td></tr>
      @endforelse
    </tbody>
  </table>
</div>
